  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-6">
        <button className="btn-primary" onClick={() => window.location.href = '/pacientes'}>
          <Plus size={16} /> Novo Paciente
        </button>
        <button className="btn-sec" onClick={() => window.location.href = '/agenda'}>
      ...
      <div className="grid grid-cols-4 max-lg:grid-cols-2 max-sm:grid-cols-1 gap-4 mb-6">
        <div className="bg-surface border border-border rounded-md p-5 hover:border-accent transition-colors">
          <Users size={20} className="mb-3" style={{ color: 'var(--accent)' }} />
          <div className="text-xs uppercase tracking-wider font-mono mb-2" style={{ color: 'var(--muted)' }}>Total de Pacientes</div>
          <div className="text-3xl font-bold font-mono tabular-nums">{kpis.pacientes}</div>
        </div>
        <div className="bg-surface border border-border rounded-md p-5 hover:border-accent transition-colors">
          <Calendar size={20} className="mb-3" style={{ color: 'var(--accent)' }} />
          <div className="text-xs uppercase tracking-wider font-mono mb-2" style={{ color: 'var(--muted)' }}>Agendamentos Hoje</div>
          <div className="text-3xl font-bold font-mono tabular-nums">{kpis.hoje}</div>
        </div>
        <div className="bg-surface border border-border rounded-md p-5 hover:border-accent transition-colors">
          <DollarSign size={20} className="mb-3" style={{ color: 'var(--accent)' }} />
          <div className="text-xs uppercase tracking-wider font-mono mb-2" style={{ color: 'var(--muted)' }}>Faturamento (mês)</div>
          <div className="text-3xl font-bold font-mono tabular-nums">R$ {kpis.faturamento.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
        </div>
        <div className="bg-surface border border-border rounded-md p-5 hover:border-accent transition-colors">
          <TrendingUp size={20} className="mb-3" style={{ color: 'var(--accent)' }} />
          <div className="text-xs uppercase tracking-wider font-mono mb-2" style={{ color: 'var(--muted)' }}>Novos Leads (semana)</div>
          <div className="text-3xl font-bold font-mono tabular-nums">{kpis.leads}</div>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-md p-5 hover:border-accent transition-colors">
        <h3 className="text-sm font-semibold mb-4" style={{ color: 'var(--fg-2)' }}>Atividades Recentes</h3>
        {atividades.length === 0 ? (
          ...
          atividades.map((a, i) => (
            <div key={i} className="flex items-center gap-3 py-3 border-b last:border-b-0" style={{ borderColor: 'var(--border)' }}>
              <div className="w-2 h-2 rounded-full" style={{ background: a.status === 'realizado' ? 'var(--success)' : a.status === 'agendado' ? 'var(--accent)' : 'var(--warn)' }} />
